...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const defaultAvatar = '/app-assets/assets/default-avatar.png';
                    ...
                    mom.forEach(m => {
                        ...
                        if (mediaUrls.length > 0) {
                            ...
                            mediaUrls.forEach(mu => {
                                media += '<img src="' + mu + '" style="width:100%;max-height:200px;object-fit:cover;border-radius:8px;cursor:pointer;" onerror="this.style.display=\'none\'">';
                            });
                            media += '</div>';
                        }
    ...
    async function initWebSocket() {
        try {
            ...
            const wsUrl = `${protocol}//${window.location.host}/v1/ws?token=${encodeURIComponent(token)}&sid=${encodeURIComponent(wsSessionId)}`;
    ...
    function createMessageElement(msg, convKey, currentSeen) {
        ...
        if (msgType === 'image') {
            ...
            if (!isSelf) {
                const avatarUrl = msg.from_avatar || msg.sender_avatar || msg.avatar_url || '/app-assets/assets/default-avatar.png';
                const avatarImg = document.createElement('img');
                avatarImg.src = avatarUrl;
                avatarImg.className = 'msg-avatar';
                avatarImg.onerror = () => { avatarImg.src = '/app-assets/assets/default-avatar.png'; };
    ...
    async function loadGroupMembers() {
        ...
        try {
            ...
            mentionMembers = (data.members || []).map(m => ({
                uid: m.uid,
                name: m.display_name || m.username || m.uid
            }));
        } catch (e) {
    ...
    function renderMentionList(list) {
        ...
        list.forEach((m, i) => {
            ...
            item.innerHTML = `<img src="${m.avatar || '/app-assets/assets/default-avatar.png'}" onerror="this.src='/app-assets/assets/default-avatar.png'"><span class="mention-name">${escapeHtml(m.name)}</span>`;
    ...
    async function showEmoticonPicker() {
        ...
        try {
            ...
            items.forEach(item => {
                ...
                itemEl.innerHTML = `<img src="${imgUrl}" loading="lazy">`;
